...
void inpt(char sym) {
	...
	do {
		cout << "Enter vnzname: "; cin >> vnz.moremoreinfo.vnzname;	//
		cout << "Enter city: "; cin >> vnz.moremoreinfo.cityname;	//
		cout << "Enter lvlakr: "; cin >> vnz.moremoreinfo.lvlakr;	//
		cout << "Enter fakultnum: "; cin >> vnz.fakultnum;			//
...
void otpt() {
	...
	while (fileout.read((char*)&vnz, sizeof(info))) {	//читання файла
		printf("\nVnzname: %-21s City: %-16s Lvlakr: %i Number of fakultets: %4i",			//форматований вивід інформації в консоль
...
void otpt2() {																				//аналогічно
	...
	while (fileout.read((char*)&vnz, sizeof(moreinfo)))
		printf("\nVnzname: %-21s City: %-16s Lvlakr: %i", vnz.vnzname, vnz.cityname, vnz.lvlakr);
	fileout.close();
}
...
void maxstud() {
	...
	cout << "\nThe most num of students is on " << faknum << " fakultet in " << vnzname << " vnz: " << studnum << " students";
	fileout.close();
}
